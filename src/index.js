...
function checksExistsUserAccount(request, response, next) {
  ...
  if (!userByUsername) {
    return response.status(404).json({ error: "Insert a valid user" });
  }
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  return response.status(403).json({ error: "Not able to complete task!" });
}
...
function checksTodoExists(request, response, next) {
  ...
  const todoBelongsToUser = userByUsername.todos.find((todo) => todo.id === id);

  if (!todoBelongsToUser) {
    return response.status(404).json({ error: "Todo is not from user" });
  }
